...
router.get('/talker/:id', async (req, res) => {
  ...
  isWatchedFormatCorrect,
  rateRequired,
  isRateFormatCorrect, async (req, res) => {
    const talkersFile = await readFile(PATH_TALKERSFILE);
    const talkersList = JSON.parse(talkersFile);
    const { name, age, talk } = req.body;
    const { watchedAt, rate } = talk;
    const newTalker = { id: talkersList.length + 1, name, age, talk: { watchedAt, rate } };
...
router.post('/login', 
  ...
  //   const objTalkers = await addTalker(PATH_TALKERSFILE, newTalker);
  //   return res.status(201).json(objTalkers);
